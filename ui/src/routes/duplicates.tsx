  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        DECLINED_GROUPS_STORAGE_KEY,
        JSON.stringify(Array.from(declinedGroups)),
      )
    } catch (error) {
      ...
      <div className="container mx-auto p-6">
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-500">
            {duplicates && duplicates.totalGroups > 0
              ? 'All visible duplicate groups have been reviewed.'
                    ...
                    >
                      {mergeMutation.isPending ? (
                        ...
                        <>
                          ...
                          {hasMoreThanTwo && hasSelection
                            ? `Merge Selected (${selectedInGroup.length})`
                            : 'Merge'}
                        </>
                      )}
                    </Button>
                ...
                <div className="space-y-3">
                  {group.contacts.map((contact) => {
                    ...
                    return (
                      <div
                        key={contact.id}
                        className="flex items-start gap-3 p-3 border rounded-lg hover:bg-muted/20"
                      >
                        {hasMoreThanTwo && (
                          <Checkbox
                            checked={isSelected}
