...
export function mergeContacts(contacts: Contact[]): Partial<Contact> {
  ...
  const merged: Partial<Contact> = {
    ...
    full_name:
      primary.full_name || sorted.find((c) => c.full_name)?.full_name || null,
    first_name:
      primary.first_name ||
      sorted.find((c) => c.first_name)?.first_name ||
      null,
    last_name:
      primary.last_name || sorted.find((c) => c.last_name)?.last_name || null,
    middle_name:
      primary.middle_name ||
      sorted.find((c) => c.middle_name)?.middle_name ||
      null,
    nickname:
      primary.nickname || sorted.find((c) => c.nickname)?.nickname || null,
    maiden_name:
      primary.maiden_name ||
      sorted.find((c) => c.maiden_name)?.maiden_name ||
      null,

    // Organization/Job Title: prefer primary, fallback to first non-null
    organization:
      primary.organization ||
      sorted.find((c) => c.organization)?.organization ||
      null,
    job_title:
      primary.job_title || sorted.find((c) => c.job_title)?.job_title || null,
    ...
    birthday:
      primary.birthday || sorted.find((c) => c.birthday)?.birthday || null,

    // Homepage: prefer primary, fallback to first non-null
    homepage:
      primary.homepage || sorted.find((c) => c.homepage)?.homepage || null,
 ...
 */
export function namesFuzzyMatch(
  name1: string | null,
  name2: string | null,
): boolean {
  const norm1 = normalizeName(name1)
...
export function detectDuplicates(contacts: Contact[]): DuplicateGroup[] {
  ...
  for (const [email, groupContacts] of emailGroups.entries()) {
    if (groupContacts.length > 1) {
      ...
      const uniqueContacts = Array.from(
        new Map(groupContacts.map((c) => [c.id, c])).values(),
      )
  ...
  for (let i = 0; i < contacts.length; i++) {
    ...
    for (let j = i + 1; j < contacts.length; j++) {
      ...
      if (processed.has(contact2.id) || fuzzyProcessed.has(contact2.id))
        continue
