...
describe('generateVCard line folding (large content regression)', () => {
	// Regression for an O(n^2) line-folding bug that OOM-crashed the process on
	// contacts with a large embedded photo (a multi-MB base64 PHOTO line). A big
	// NOTE exercises the same foldVCardLine path. The old implementation could
	// not complete this; it must now be fast and correct.
	it('folds a multi-megabyte line without exhausting memory', () => {
		const big = 'x'.repeat(3_000_000)
		const result = generateVCard({ uid: 'big-note', fn: 'Big Note', version: '3.0', note: big })
